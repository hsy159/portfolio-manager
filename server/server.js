...
app.get('/api/exchange', async (req, res) => {
  ...
  try {
    ...
    res.json({ rates: [{ pair: `${from}${to}`, rate: q.regularMarketPrice, change: q.regularMarketChange, changePercent: q.regularMarketChangePercent }], fetchedAt: new Date().toISOString() });
  } catch (err) {
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const results = (data.quotes || []).map(r => ({ symbol: r.symbol, name: r.shortname || r.longname || r.symbol, type: r.quoteType, exchange: r.exchange }));
...
app.post('/api/portfolio-quote', async (req, res) => {
  ...
  try {
    ...
    res.json({
      holdings: results,
      summary: { totalValueKRW: tv, totalCostKRW: tc, totalProfitKRW: tv - tc, totalReturnPct: Math.round(((tv - tc) / tc) * 10000) / 100, holdingsCount: results.length },
